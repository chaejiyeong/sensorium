...
for i in range(1):
    ...
    gsknn = GridSearchCV(knn, params, cv=3,
    ...
    # y_hat = knn.predict(X_test2)
    # matrix = metrics.confusion_matrix(y_test, y_hat)
    """
    parameter_array.append(gsknn.best_params_)
    estimator = gsknn.best_estimator_
...
"""
# df = pd.DataFrame(acc)
# df.to_csv('KNN_실험2.csv')
sys.stdout = open('KNN_2_result.txt', 'w')
print(acc)
sys.stdout.close
