...
for i in range(100):
    ...
    mlp = MLPClassifier(random_state=i, activation='logistic', max_iter=500)
    dnn = MLPClassifier(random_state=i, activation='logistic',
                        max_iter=500, hidden_layer_sizes=(100, 100))
    gb = GradientBoostingClassifier(
        random_state=i, criterion='friedman_mse', learning_rate=0.1, max_depth=8, max_features='sqrt')
    vo_clf = VotingClassifier(
        estimators=[('GB', gb), ('DNN', dnn), ('MLP', mlp)], voting='soft')
    vo_clf.fit(X_train, y_train)
    y_hat = vo_clf.predict(X_test2)
    acc.append(accuracy_score(y_test2, y_hat, normalize=True))
    """
